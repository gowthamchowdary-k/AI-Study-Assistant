...
export default function RegisterPage({ onRegisterSuccess, onNavigateToLogin }) {
    ...
    async function handleSubmit(e) {
        ...
        try {
            setLoading(true);
            const response = await registerUser(cleanEmail, password);
            if (response.success && response.token) {
                localStorage.setItem("token", response.token);
                toast.success("Account created! Welcome to AI Study Assistant 🎉");
            ...
            >
                <div className="bg-slate-900/60 backdrop-blur-xl border border-slate-800 rounded-3xl p-8 shadow-2xl">
                    ...
                    <div className="flex flex-col items-center mb-8">
                        ...
                        <h2 className="text-2xl font-bold text-white tracking-tight">Join AI Study Assistant</h2>
                        <p className="text-slate-400 text-sm mt-1">Create an account to start studying smarter</p>
                    </div>
